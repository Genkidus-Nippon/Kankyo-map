/* =========================================================
   ...
   ※厳密な統計ではなく傾向を示す「代表値モデル」。
     色は各年代で固定スケール(colorMax)を使うため、
     年代を動かすと 1950=薄い → 2020=濃い がはっきり変化します。
   ========================================================= */
...
const METRICS = {
  warming: {
    ...
    model:(a,t)=> +(a*t).toFixed(2),                 // 1950=0 → 2020=a（変化が最も明確）
    anchor:{
  ...
  desert: {
    ...
    note:"国土に占める乾燥地の割合の概算（年代とともに拡大）",
    lowLabel:"低い", highLabel:"高い",
    model:(a,t)=> +Math.min(100, a*(0.6+0.4*t)).toFixed(0),  // 1950=0.6a → 2020=a
    anchor:{
      ...
      "Iran":85,"Botswana":84,"Afghanistan":80,"Sudan":80,"Uzbekistan":80,"Mongolia":78,
      "Tunisia":75,"Iraq":75,"Pakistan":75,"Australia":70,"Morocco":70,"Kazakhstan":66,
      "Syria":60,"Mexico":52,"Kenya":50,"South Africa":50,"Ethiopia":45,"Argentina":40,
      "India":32,"Spain":32,"China":30,"United States of America":30,"Turkey":30,"Russia":15,
    },
  },
  forest: {
    ja:"森林減少（1950年比）", unit:"%",
    source:"代表値モデル（FAO等の傾向に基づく概算）",
    note:"1950年の森林面積からの累積減少率（マイナスは増加＝再植林傾向）",
    lowLabel:"少ない/増加", highLabel:"多い",
    model:(a,t)=> +(a*t).toFixed(1),                 // 1950=0 → 2020=a
    anchor:{
      "Ivory Coast":45,"Nigeria":40,"Madagascar":40,"Ghana":33,"Honduras":30,"Philippines":30,
      "Indonesia":26,"Cambodia":26,"Paraguay":25,"Thailand":25,"Guatemala":25,"Malaysia":24,
      "Laos":22,"Brazil":22,"Myanmar":20,"Argentina":20,"Ethiopia":20,"Tanzania":18,
  ...
  water: {
    ...
    note:"取水量/利用可能水量に基づく水ストレスの概算（年代とともに上昇）",
    lowLabel:"低い", highLabel:"高い",
    model:(a,t)=> +Math.min(100, a*(0.5+0.5*t)).toFixed(0),  // 1950=0.5a → 2020=a
    anchor:{
  ...
  air: {
    ...
    lowLabel:"低い", highLabel:"高い",
    model:(a,t)=> +(a*(0.4+0.6*t)).toFixed(0),        // 1950=0.4a → 2020=a
    anchor:{
...
function colorMaxOf(m){
  let mx = 0.0001;
  for (const a of Object.values(m.anchor)) mx = Math.max(mx, m.model(a, 1));
  return mx;
}
...
function getTheme(metric, decade){
  ...
  return {
    ...
    colorMax: colorMaxOf(m),      // ← 固定スケール（年代で色が変わる）
  };
}
